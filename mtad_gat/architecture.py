# code based on https://github.com/srigas/MTAD-GAT-mlflow/blob/main/architecture.py
# added "anomaly" mode to allow direct invocations of MTAD_GAT by mlserver
...
class MTAD_GAT(nn.Module):
    ...
    def forward(self, 
            x,  # input with shape (b, n, k): b - batch size, n - window size, k - number of features
            mode: str = "anomaly", # pred, recon, or anomaly
            threshold: float = 1.0 # threshold for anomaly detection
    ):
       
        if mode == "anomaly":
            # run for two passes and then compare prediction & reconstruction
            # equal to score + predict in https://github.com/srigas/MTAD-GAT-mlflow/blob/main/model.py 
            
            pred_x = x[:, :-1, :]
            predictions = self.forward(pred_x, mode="pred")
            
            recon_x = x[:, 1:, :]
            recons = self.forward(recon_x, mode="recon")
            ...
            recons = recons[:, -1, :].detach().cpu().numpy()
            actual = actual.detach().cpu().numpy()
        
            anomaly_scores = (np.sqrt((predictions[:,:]-actual[:,:])**2) + np.sqrt(
                            (recons[:,:]-actual[:,:])**2))/2.0
                            
            anomaly_scores = np.sum(anomaly_scores, 1)

            # normalize score to indicate an anomaly with a value greater than 1
            normalized_score = [score / threshold for score in anomaly_scores]
